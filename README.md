...
- `scripts/validate-ui.sh`: Playwright 기반 실제 브라우저 UI 검증
- `scripts/validate-db.sh`: DB 통합 검증 placeholder

아직 실제 DB migration 실행과 Docker Postgres 기동은 연결되어 있지 않습니다. UI 검증은 Playwright로 연결되어 있으며, 현재 스캐폴딩 UI가 Figma 디자인과 다르면 실패합니다.
...
주의: Figma-to-Web 구현이 완료되기 전에는 `validate-ui.sh`가 실패할 수 있습니다. 이 실패는 밝은 임시 UI가 남아 있다는 신호이며, 하네스 자체의 실패가 아닙니다.

## UI 검증

UI 검증 스크립트는 Playwright로 실제 브라우저를 실행합니다.
...
- `apps/web`의 Next dev server를 실행하거나 기존 `localhost:3000` 서버를 재사용합니다.
- 데스크톱/모바일 Chromium viewport에서 `/` 화면을 검증합니다.
- Figma 기준의 다크 정보형 UI 톤이 아니면 실패합니다.
- 현재 같은 밝은 스캐폴딩/placeholder UI가 남아 있으면 실패합니다.
- 가로 overflow, 주요 버튼, 필터/패널, 모집 카드 밀도를 확인합니다.

필요한 브라우저가 없으면 먼저 설치합니다.

```bash
pnpm --filter @tft-doubleup/web exec playwright install chromium
```

Figma 기준 UI를 구현할 때는 `.agents/skills/tft-doubleup-figma-to-web-implementation/SKILL.md`를 따른 뒤 `./scripts/check.sh`와 `./scripts/validate-ui.sh`를 모두 통과시킵니다.
...
`@playwright/test` 의존성과 Chromium 브라우저 설치 상태를 확인합니다.
...
현재 화면이 Figma 기준 다크 정보형 UI가 아니면 `validate-ui.sh`가 실패합니다. 이 경우 테스트를 완화하지 말고 `apps/web` 구현을 Figma 기준으로 수정해야 합니다.
